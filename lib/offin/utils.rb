...
class Utils
  ...
  def self.ingest_usage
    program = $0.sub(/.*\//, '')
    STDERR.puts "Usage: #{program} <directory>"
    exit 1
  end

  def self.xml_unescape str
  ...
  def self.field_system_error message = ''
    yield
  rescue => e
  ...
  def self.silence_streams(*streams)  # after some rails code by DHH
  ...
  def self.pretty_elapsed elapsed
  ...
  def self.quickly time = QUICKLY_TIMEOUT
  ...
  def self.get_pre_existing_islandora_pid_for_iid config, iid
    ...
    url = "#{config.solr_url}/select/?q=mods_identifier_iid_mls:#{self.solr_escape(iid)}&version=2.2&indent=on&fl=PID,mods_identifier_iid_ms"
    uri = URI.encode(url)
  ...
  def self.get_collection_names config
  ...
  def self.get_next_newspaper_issue_sequence config, newspaper_pid
    query = <<-SPARQL.gsub(/^        /, '')
        PREFIX islandora-rels-ext: <http://islandora.ca/ontology/relsext#>
        PREFIX fedora-rels-ext: <info:fedora/fedora-system:def/relations-external#>
  ...
  def self.get_newspaper_issues_by_date_issued config, newspaper_pid, issue_date
  ...
  def self.get_newspaper_pids config
  ...
  def self.get_datastream_names config, pid
  ...
  def self.ping_islandora_for_object islandora_site, pid
  ...
  def self.ping_islandora_for_objects islandora_site, pids
    map = {}
    pids.each do |pid|
      map[pid] = self.ping_islandora_for_object islandora_site, pid
    end
    return map
  end

  def self.get_manifest config, directory
  ...
  def self.get_mods config, directory
  ...
  def self.temp_file         # creat an anonymous file handle
  ...
  def self.format_error_messages text
  ...
  def self.image_processing config, image_filepath, command, error_title
    ...
    errors = self.format_error_messages(error_text)
    errors.unshift error_title unless (errors.nil? or errors.empty?)

    return image, errors
  end

  def self.pass_through filepath
  ...
  def self.size config, image_filepath
    file, errors = self.image_processing(config, image_filepath,
                                          "#{config.image_convert_command} -identify #{self.shellescape(image_filepath)} null:",
  ...
  def self.pdf_to_thumbnail config, pdf_filepath
    return self.image_processing(config, pdf_filepath,
                                  "#{config.pdf_convert_command} -resize #{config.thumbnail_geometry} #{self.shellescape(pdf_filepath + '[0]')} jpg:-",
  ...
  def self.pdf_to_preview config, pdf_filepath
    return self.image_processing(config, pdf_filepath,
                                  "#{config.pdf_convert_command} -resize #{config.pdf_preview_geometry} #{self.shellescape(pdf_filepath + '[0]')} jpg:-",
  ...
  def self.pdf_to_text config, pdf_filepath
    return self.image_processing(config, pdf_filepath,
                                  "#{config.pdf_to_text_command} #{self.shellescape(pdf_filepath)} -",
  ...
  def self.jp2k_ok? config, image_filepath
    file, errors = self.image_processing(config, image_filepath, "#{config.image_convert_command} -identify #{self.shellescape(image_filepath)} null:", "")
  ...
  def self.convert_jp2k_maybe config, image_filepath
    unused = nil
    yield image_filepath, nil unless self.mime_type(image_filepath) == 'image/jp2'
    yield image_filepath, nil if self.jp2k_ok?(config, image_filepath)

    temp_image_filepath = Tempfile.new('image-kakadu-').path + '.tiff'
    unused, errors = self.image_processing(config, image_filepath,
                                            "#{config.kakadu_expand_command} -i #{self.shellescape(image_filepath)} -o #{self.shellescape(temp_image_filepath)}",
  ...
  def self.image_to_jpeg config,  image_filepath
    return self.pass_through(image_filepath) if self.mime_type(image_filepath) == 'image/jpeg'
    working_image_filepath = nil
    self.convert_jp2k_maybe(config, image_filepath) do |working_image_filepath, errors|
      return open('/dev/null'), errors unless errors.nil? or errors.empty?
      return self.image_processing(config, working_image_filepath,
                                    "#{config.image_convert_command} #{self.shellescape(working_image_filepath)} jpeg:-",
  ...
  def self.image_to_pdf config, image_filepath
    return self.pass_through(image_filepath) if self.mime_type(image_filepath) == 'application/pdf'
    working_image_filepath = nil
    self.convert_jp2k_maybe(config, image_filepath) do |working_image_filepath, errors|
      return open('/dev/null'), errors unless errors.nil? or errors.empty?
      return self.image_processing(config, working_image_filepath,
                                    "#{config.image_convert_command} #{self.shellescape(working_image_filepath)} pdf:-",
  ...
  def self.image_to_tiff config,  image_filepath
    return self.pass_through(image_filepath) if self.mime_type(image_filepath) == 'image/tiff'
    working_image_filepath = nil
    self.convert_jp2k_maybe(config, image_filepath) do |working_image_filepath, errors|
      return open('/dev/null'), errors unless errors.nil? or errors.empty?
      return self.image_processing(config, working_image_filepath,
                                    "#{config.image_convert_command} #{self.shellescape(working_image_filepath)} tiff:-",
  ...
  def self.image_resize config, image_filepath, geometry, new_format = nil
  ...
  def self.langs_to_tesseract_command_line config, *requested_languages
  ...
  def self.langs_to_names config, *requested_languages
    supported = config.supported_ocr_languages
    wanted = []
  ...
  def self.langs_unsupported_comment config, *requested_languages
    supported = config.supported_ocr_languages
    unsupported = []
  ...
  def self.langs_supported config, requested_languages
  ...
  def self.tesseract config, image_filepath, do_hocr, *langs
    ...
    if self.mime_type(image_filepath) == 'image/jp2'
      if self.jp2k_ok?(config, image_filepath)
        temphandle, errors =  self.image_processing(config, image_filepath,
                                                     "#{config.image_convert_command} #{self.shellescape(image_filepath)} tiff:-",
        ...
        unused, errors = self.image_processing(config, image_filepath,
                                                    "#{config.kakadu_expand_command} -i #{self.shellescape(image_filepath)} -o #{self.shellescape(tiff_filepath)}",
                                                    "Failed attempt to convert #{image_filepath} using kakadu after JP2 image failure.")
        return nil, errors unless errors.nil? or errors.empty?
      end
    ...
    cmdline = config.tesseract_command + ' ' + self.langs_to_tesseract_command_line(config, *langs)   + ' ' + self.shellescape(image_filepath) +  ' ' + base_filename + (do_hocr ? ' hocr' : '')
  ...
  def self.hocr config, image_filepath, *langs
    text, errors = self.tesseract(config, image_filepath, true, *langs)
  ...
  def self.ocr config, image_filepath, *langs
    text, errors = self.tesseract(config, image_filepath, false, *langs)
    return unless text.class == String
    return if text.empty?
    return text
  end
  ...
  def self.mime_type file
    ...
    raise PackageError, "self.mime_type error: #{error}" if not error.empty?
    return type.strip
  end
  ...
  def self.available_collection_codes
  ...
  def self.setup_basic_filters sql, params

    from, to = self.parse_dates(params['from'], params['to'])
  ...
  def self.get_datastream_contents config, pid, dsid
  ...
  def self.rels_ext_get_policy_fields config, collection_pid
    str = <<-XML
        <islandora:inheritXacmlFrom rdf:resource="info:fedora/#{collection_pid}"/>
    XML

    rels_ext_content = self.get_datastream_contents(config, collection_pid, 'RELS-EXT')
  ...
  def self.find_appropriate_admin_config config_file, server_name
